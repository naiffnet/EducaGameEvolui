/**
 * Utilitário de senha — Entrega I (Autenticação com Senha Real)
 *
 * IMPORTANTE: isto NÃO é segurança de nível de produção. Este projeto não tem backend
 * (SPA + localStorage), então não há como fazer hashing do lado do servidor, salt seguro
 ...
 * de seed data, sem reestruturar a inicialização do banco para ser assíncrona).
 */
...
/**
 * Gera o hash de uma senha. O e-mail é usado como sal fixo por conta (não é um sal
 * criptográfico seguro — apenas garante que a mesma senha gere hashes diferentes para
 * contas diferentes, o que já é estritamente melhor que nenhum sal).
 */
export function hashPassword(password: string, email: string): string {
  return sha256Hex(`${password}:${email.trim().toLowerCase()}`);
}
