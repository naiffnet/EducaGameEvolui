...
- **Regra de Negócio B1 (idempotência):** a chave natural `studentId + date` é única — registrar a chamada do mesmo aluno na mesma data **atualiza** o registro existente (não duplica). O XP só é concedido **uma única vez por `studentId + date`, para sempre** — controlado por um campo explícito `xpGranted` no próprio registro (que uma vez `true` nunca volta a `false`), não inferido do valor atual de `present`. Isso evita tanto o "clique duplo" (User Story 5) quanto o caso mais sutil de alternar presente/falta/presente no mesmo dia para tentar farmar XP repetido.
